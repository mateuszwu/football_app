...
RSpec.describe Players::ProfileStatsQuery do
  describe ".call" do
    it "returns season-scoped profile stats, charts, awards, pagination, and player-filtered synergy links" do
      ...
      expect(result.record).to include(wins: 1, draws: 0, losses: 0, total: 1, win_rate: 100)
      ...
      expect(result.chart_data.fetch(:elo).fetch(:datasets).first.fetch(:data)).to eq([ 1040 ])
